...
public class SwaggerConfiguration implements WebMvcConfigurer{
	...
	@Bean
	public Docket requestStatApi() {
		return new Docket(DocumentationType.SWAGGER_2)
				.select()                 
				.apis(RequestHandlerSelectors.basePackage("com.adex.filterservice.controller"))
				.paths(PathSelectors.regex(".*/*"))
				.build()
				.apiInfo(requestStatApiMetaData());

	}

	private ApiInfo requestStatApiMetaData() {
		return new ApiInfoBuilder()
				.title("Request Statistics REST API")
				.description("API for collecting request statistics.")
				.version(appVersion)
